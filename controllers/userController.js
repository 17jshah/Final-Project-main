...
exports.profilePage = (req, res, next) => {
  ...
  Promise.all([
    User.findById(id),
    Connection.find({ hostName: id }),
    model_rsvp
      .find({ user: id }, { Connection: 1, commitment: 1 })
      .populate("connection", "title category"),
  ])
    .then((result) => {
      const [profile, connections, rsvp_connections] = result;
      console.log("rsvp connections=>", rsvp_connections);
      res.render("./user/profile", { profile, connections, rsvp_connections });
    })
    .catch((err) => next(err));
